...
$helpers = @{
    ConvertFromGitToDocs = { 
        ...
        $tmpGitPath = $gitPath -replace "$($Matches[1])","https://docs.microsoft.com/en-us/azure-stack/"  
...
$template = @'
---
title: Azure Stack Update at <%= $date %>
date: <%= $date %>
draft: false
tags: [
]
...
$latestTwo = Get-AzStorageBlob -Container '$web' -Context $storageAccount.Context -Prefix "azurestack/" | Sort-Object LastModified -Descending -Top 2
...
if ($latestList.sha -ne $previousList.sha){
    ...
    $apiPath = "/contents/content/posts/azs-update-$date.md"
